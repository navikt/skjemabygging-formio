...
if (printHelp) {
  ...
  console.log();
  console.log(
    'Finds all package.json files recursively under given path, and writes a list of objects with package name and path to stdout.',
  );
...
function parsePackageJson(filename) {
  const jsonDataString = fs.readFileSync(filename, { encoding: 'utf-8', flag: 'r' });
