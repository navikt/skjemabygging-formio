  ...
  verifyToken: (token: string): JwtPayload | null => {
    try {
      ...
      if (err instanceof jwt.TokenExpiredError) {
        logger.info('Nologin token has expired', { errorMessage: err.message, correlationId: correlator.getId() });
        return null;
      }
      if (err instanceof jwt.JsonWebTokenError) {
        logger.warn('Failed to verify nologin token', { errorMessage: err.message, correlationId: correlator.getId() });
        return null;
      }
      throw err;
    }
