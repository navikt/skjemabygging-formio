...
describe('Native render path', () => {
  beforeEach(() => {
    cy.intercept('GET', '/fyllut/api/config*', (req) => {
      req.continue((res) => {
        res.body.nativeRenderForms = ['nativerender'];
      });
    }).as('getConfig');
  ...
  it('renders inputs, validates per page, and navigates', () => {
    cy.visit('/fyllut/nativerender?sub=paper');

    cy.findByRole('heading', { name: 'Page one', timeout: 20000 }).should('exist');
    ...
    cy.location('pathname').should('eq', '/fyllut/nativerender');
  ...
  it('saves a draft on next and submits from the summary (digital)', () => {
    ...
    cy.visit('/fyllut/nativerender?sub=digital');
    ...
    cy.findByRole('button', { name: 'Til oppsummering' }).click();
