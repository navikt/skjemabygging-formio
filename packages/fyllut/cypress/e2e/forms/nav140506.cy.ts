...
const completePlanWithSharedPeriod = () => {
  ...
  cy.findByRole('textbox', { name: /Fellesperiode fra og med/i }).type('01.02.2025');
  cy.findByRole('textbox', { name: /Fellesperiode til og med/i }).type('15.02.2025');
...
describe('NAV 14-05.06 - production fixture parity', () => {
  beforeEach(() => {
    cy.visitFixtureForm('nav020805', { fixture: 'forms/nav140506.json' });
  });

  it('keeps medmor shared-period conditionals and attachment parity aligned on the summary page', () => {
    completeIntroAndApplicant();

    answerRadio(/Hvem er du/i, /^Medmor$/i);
    answerRadio(/Hvor lang periode med foreldrepenger ønsker du/i, /100 prosent/i);
    cy.clickNextStep();

    answerRadio(/Gjelder søknaden din stebarnsadopsjon/i, /^Nei$/i);
    cy.findByRole('textbox', { name: /Hvor mange barn skal du adoptere/i }).type('1');
    cy.findByRole('textbox', { name: /Når overtar du omsorgen/i }).type('01.01.2025');
    cy.findByRole('textbox', { name: /Når ble barnet født/i }).type('01.01.2024');
    cy.clickNextStep();
