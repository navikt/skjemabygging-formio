...
describe("[api router endpoint] send-inn", () => {
  const defaultBody = {
  ...
  it("returns 201 and location header if success", async () => {
    nock(config.sendInnHost).post("/fyllUt/leggTilVedlegg").reply(302, "FOUND", { Location: SEND_LOCATION });
  ...
  it("calls next if SendInn returns error", async () => {
    nock(config.sendInnHost).post("/fyllUt/leggTilVedlegg").reply(500, "error body");
    const req = mockRequest({ body: defaultBody });
    req.getIdportenPid = () => "12345678911";
    const res = mockResponse();
    const next = jest.fn();
