const currencies = {
  betydninger: {
    FJD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Fiji",
            tekst: "Dollars Fiji",
          },
        },
      },
    ],
    STD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dobras SÒo Tomeand Principe",
            tekst: "Dobras SÒo Tomeand Principe",
          },
        },
      },
    ],
    MXN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Mexico",
            tekst: "Mexico",
          },
        },
      },
    ],
    SCR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rupees Seychelles",
            tekst: "Rupees Seychelles",
          },
        },
      },
    ],
    TVD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tuvalu Dollars Tuvalu",
            tekst: "Tuvalu Dollars Tuvalu",
          },
        },
      },
    ],
    LVL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Latvia",
            tekst: "Latvia",
          },
        },
      },
    ],
    CDF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Congolese Francsá Congo/Kinshas",
            tekst: "Congolese Francsá Congo/Kinshas",
          },
        },
      },
    ],
    BBD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Barbados",
            tekst: "Dollars Barbados",
          },
        },
      },
    ],
    GTQ: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Quetzales Guatemala",
            tekst: "Quetzales Guatemala",
          },
        },
      },
    ],
    CLP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Chile",
            tekst: "Pesos Chile",
          },
        },
      },
    ],
    UGX: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Shillings Uganda",
            tekst: "Shillings Uganda",
          },
        },
      },
    ],
    HNL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Lempiras Honduras",
            tekst: "Lempiras Honduras",
          },
        },
      },
    ],
    ZAR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sør-Afrikanse rand",
            tekst: "Sør-Afrikanse rand",
          },
        },
      },
    ],
    TND: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tunis",
            tekst: "Tunis",
          },
        },
      },
    ],
    SLL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Leones SierraLeone",
            tekst: "Leones SierraLeone",
          },
        },
      },
    ],
    BSD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Bahamas",
            tekst: "Dollars Bahamas",
          },
        },
      },
    ],
    SDD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sudan dinar",
            tekst: "Sudan dinar",
          },
        },
      },
    ],
    IQD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dinars Iraq",
            tekst: "Dinars Iraq",
          },
        },
      },
    ],
    CUP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Cuba",
            tekst: "Pesos Cuba",
          },
        },
      },
    ],
    GMD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dalasi Gambia",
            tekst: "Dalasi Gambia",
          },
        },
      },
    ],
    TWD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Taiwan",
            tekst: "Taiwan",
          },
        },
      },
    ],
    DOP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Dominican Republic",
            tekst: "Pesos Dominican Republic",
          },
        },
      },
    ],
    KMF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Francs Comoros",
            tekst: "Francs Comoros",
          },
        },
      },
    ],
    MYR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Malaysia Ringgit",
            tekst: "Malaysia Ringgit",
          },
        },
      },
    ],
    FKP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Falkland Islands (Malvin",
            tekst: "Pounds Falkland Islands (Malvin",
          },
        },
      },
    ],
    XOF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Benin,BurkinaFaso,IvoryCoast,Mali,Niger,Senega",
            tekst: "Benin,BurkinaFaso,IvoryCoast,Mali,Niger,Senega",
          },
        },
      },
    ],
    GEL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Lari Georgia",
            tekst: "Lari Georgia",
          },
        },
      },
    ],
    UYU: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Uruguay",
            tekst: "Pesos Uruguay",
          },
        },
      },
    ],
    MAD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Marokko",
            tekst: "Marokko",
          },
        },
      },
    ],
    CVE: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Escudos Cape Verde",
            tekst: "Escudos Cape Verde",
          },
        },
      },
    ],
    AZM: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Manats Azerbaijan",
            tekst: "Manats Azerbaijan",
          },
        },
      },
    ],
    TOP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pa'anga Tonga",
            tekst: "Pa'anga Tonga",
          },
        },
      },
    ],
    OMR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rials Oman",
            tekst: "Rials Oman",
          },
        },
      },
    ],
    PGK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kina Papua New Guinea",
            tekst: "Kina Papua New Guinea",
          },
        },
      },
    ],
    SEK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Svenske kroner",
            tekst: "Svenske kroner",
          },
        },
      },
    ],
    KES: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kenya",
            tekst: "Kenya",
          },
        },
      },
    ],
    BTN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Ngultrum Bhutan",
            tekst: "Ngultrum Bhutan",
          },
        },
      },
    ],
    UAH: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Hryvnia Ukraine",
            tekst: "Hryvnia Ukraine",
          },
        },
      },
    ],
    GNF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Francs Guinea",
            tekst: "Francs Guinea",
          },
        },
      },
    ],
    ERN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Nakfa Eritrea",
            tekst: "Nakfa Eritrea",
          },
        },
      },
    ],
    SVC: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Colones El Salvador",
            tekst: "Colones El Salvador",
          },
        },
      },
    ],
    MZM: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Meticais Mozambique",
            tekst: "Meticais Mozambique",
          },
        },
      },
    ],
    ARS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Argentina",
            tekst: "Pesos Argentina",
          },
        },
      },
    ],
    QAR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Qatar",
            tekst: "Qatar",
          },
        },
      },
    ],
    IRR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rials Iran",
            tekst: "Rials Iran",
          },
        },
      },
    ],
    MRO: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Ouguiyas Mauritania",
            tekst: "Ouguiyas Mauritania",
          },
        },
      },
    ],
    UZS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sums Uzbekistan",
            tekst: "Sums Uzbekistan",
          },
        },
      },
    ],
    CNY: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kina",
            tekst: "Kina",
          },
        },
      },
    ],
    THB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Thailandske baht",
            tekst: "Thailandske baht",
          },
        },
      },
    ],
    BDT: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Taka Bangladesh",
            tekst: "Taka Bangladesh",
          },
        },
      },
    ],
    LYD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dinars Libya",
            tekst: "Dinars Libya",
          },
        },
      },
    ],
    BMD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Bermuda",
            tekst: "Dollars Bermuda",
          },
        },
      },
    ],
    PHP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Filippinene",
            tekst: "Filippinene",
          },
        },
      },
    ],
    KWD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kuwait",
            tekst: "Kuwait",
          },
        },
      },
    ],
    RUB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Russland",
            tekst: "Russland",
          },
        },
      },
    ],
    PYG: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Guarani Paraguay",
            tekst: "Guarani Paraguay",
          },
        },
      },
    ],
    JMD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Jamaica",
            tekst: "Dollars Jamaica",
          },
        },
      },
    ],
    ISK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Islandske kroner",
            tekst: "Islandske kroner",
          },
        },
      },
    ],
    COP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pesos Colombia",
            tekst: "Pesos Colombia",
          },
        },
      },
    ],
    USD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Amerikanske dollar",
            tekst: "Amerikanske dollar",
          },
        },
      },
    ],
    MKD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Denars Macedonia",
            tekst: "Denars Macedonia",
          },
        },
      },
    ],
    DZD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Algerie Dinarer",
            tekst: "Algerie Dinarer",
          },
        },
      },
    ],
    PAB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Balboa Panama",
            tekst: "Balboa Panama",
          },
        },
      },
    ],
    GGP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Guernsey",
            tekst: "Pounds Guernsey",
          },
        },
      },
    ],
    SGD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Singapore dollar",
            tekst: "Singapore dollar",
          },
        },
      },
    ],
    VEB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Bolivares Venezuela",
            tekst: "Bolivares Venezuela",
          },
        },
      },
    ],
    ETB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Birr Ethiopia",
            tekst: "Birr Ethiopia",
          },
        },
      },
    ],
    JEP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Jersey",
            tekst: "Pounds Jersey",
          },
        },
      },
    ],
    VUV: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Vatu Vanuatu",
            tekst: "Vatu Vanuatu",
          },
        },
      },
    ],
    KGS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Soms Kyrgyzstan",
            tekst: "Soms Kyrgyzstan",
          },
        },
      },
    ],
    SOS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Shillings Somalia",
            tekst: "Shillings Somalia",
          },
        },
      },
    ],
    LAK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kips Laos",
            tekst: "Kips Laos",
          },
        },
      },
    ],
    ZMK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kwacha Zambia",
            tekst: "Kwacha Zambia",
          },
        },
      },
    ],
    BND: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Brunei Darussalam",
            tekst: "Dollars Brunei Darussalam",
          },
        },
      },
    ],
    XAF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Francs CommunautÚ FinanciÞre Af",
            tekst: "Francs CommunautÚ FinanciÞre Af",
          },
        },
      },
    ],
    LRD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Liberia",
            tekst: "Dollars Liberia",
          },
        },
      },
    ],
    CHF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sveitsiske franc",
            tekst: "Sveitsiske franc",
          },
        },
      },
    ],
    HRK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kuna Croatia",
            tekst: "Kuna Croatia",
          },
        },
      },
    ],
    ALL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Leke Albania",
            tekst: "Leke Albania",
          },
        },
      },
    ],
    DJF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Francs Djibouti",
            tekst: "Francs Djibouti",
          },
        },
      },
    ],
    GHC: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Cedis Ghana",
            tekst: "Cedis Ghana",
          },
        },
      },
    ],
    MTL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Malta",
            tekst: "Malta",
          },
        },
      },
    ],
    TZS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tanzania",
            tekst: "Tanzania",
          },
        },
      },
    ],
    VND: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dong Vietnam",
            tekst: "Dong Vietnam",
          },
        },
      },
    ],
    SPL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Luigini Seborga",
            tekst: "Luigini Seborga",
          },
        },
      },
    ],
    AUD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Australske dollar",
            tekst: "Australske dollar",
          },
        },
      },
    ],
    ILS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Israel",
            tekst: "Israel",
          },
        },
      },
    ],
    GYD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Guyana",
            tekst: "Guyana",
          },
        },
      },
    ],
    KPW: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Won Korea (North)",
            tekst: "Won Korea (North)",
          },
        },
      },
    ],
    MDL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Lei Moldova",
            tekst: "Lei Moldova",
          },
        },
      },
    ],
    BOB: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Bolivianos Bolivia",
            tekst: "Bolivianos Bolivia",
          },
        },
      },
    ],
    KHR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Riels Cambodia",
            tekst: "Riels Cambodia",
          },
        },
      },
    ],
    IDR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Indonesia",
            tekst: "Indonesia",
          },
        },
      },
    ],
    KYD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Cayman Islands",
            tekst: "Dollars Cayman Islands",
          },
        },
      },
    ],
    AMD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Drams Armenia",
            tekst: "Drams Armenia",
          },
        },
      },
    ],
    SHP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Saint Helena",
            tekst: "Pounds Saint Helena",
          },
        },
      },
    ],
    TRY: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tyrkia",
            tekst: "Tyrkia",
          },
        },
      },
    ],
    BWP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pulas Botswana",
            tekst: "Pulas Botswana",
          },
        },
      },
    ],
    CYP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kypros",
            tekst: "Kypros",
          },
        },
      },
    ],
    LBP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Lebanon",
            tekst: "Pounds Lebanon",
          },
        },
      },
    ],
    TJS: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Somoni Tajikistan",
            tekst: "Somoni Tajikistan",
          },
        },
      },
    ],
    JOD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dinars Jordan",
            tekst: "Dinars Jordan",
          },
        },
      },
    ],
    RWF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rwanda Francs Rwanda",
            tekst: "Rwanda Francs Rwanda",
          },
        },
      },
    ],
    HKD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Hong Kong Dollar",
            tekst: "Hong Kong Dollar",
          },
        },
      },
    ],
    AED: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Emiratene",
            tekst: "Emiratene",
          },
        },
      },
    ],
    EUR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Euro",
            tekst: "Euro",
          },
        },
      },
    ],
    LSL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Maloti Lesotho",
            tekst: "Maloti Lesotho",
          },
        },
      },
    ],
    DKK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Danske kroner",
            tekst: "Danske kroner",
          },
        },
      },
    ],
    ZWD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Zimbabwe",
            tekst: "Zimbabwe",
          },
        },
      },
    ],
    CAD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kanadiske dollar",
            tekst: "Kanadiske dollar",
          },
        },
      },
    ],
    BGN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Leva Bulgaria",
            tekst: "Leva Bulgaria",
          },
        },
      },
    ],
    MMK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kyats Myanmar (Burma)",
            tekst: "Kyats Myanmar (Burma)",
          },
        },
      },
    ],
    NOK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Norske kroner",
            tekst: "Norske kroner",
          },
        },
      },
    ],
    MUR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rupees Mauritius",
            tekst: "Rupees Mauritius",
          },
        },
      },
    ],
    SYP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Syria",
            tekst: "Pounds Syria",
          },
        },
      },
    ],
    IMP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Isle of Man",
            tekst: "Pounds Isle of Man",
          },
        },
      },
    ],
    ROL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Lei Romania",
            tekst: "Lei Romania",
          },
        },
      },
    ],
    GIP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Gibraltar",
            tekst: "Pounds Gibraltar",
          },
        },
      },
    ],
    LKR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sri Lanka",
            tekst: "Sri Lanka",
          },
        },
      },
    ],
    NGN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Nairas Nigeria",
            tekst: "Nairas Nigeria",
          },
        },
      },
    ],
    CZK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tjekkia",
            tekst: "Tjekkia",
          },
        },
      },
    ],
    CRC: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Colones CostaRica",
            tekst: "Colones CostaRica",
          },
        },
      },
    ],
    PKR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pakistanske Roupies",
            tekst: "Pakistanske Roupies",
          },
        },
      },
    ],
    ANG: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Guilders Netherlands Antilles",
            tekst: "Guilders Netherlands Antilles",
          },
        },
      },
    ],
    HTG: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Gourdes Haiti",
            tekst: "Gourdes Haiti",
          },
        },
      },
    ],
    AFA: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Afghanis Afghanistan",
            tekst: "Afghanis Afghanistan",
          },
        },
      },
    ],
    SIT: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tolars Slovenia",
            tekst: "Tolars Slovenia",
          },
        },
      },
    ],
    BHD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Bahrain dinarer",
            tekst: "Bahrain dinarer",
          },
        },
      },
    ],
    SRD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Suriname",
            tekst: "Dollars Suriname",
          },
        },
      },
    ],
    KZT: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tenge Kazakstan",
            tekst: "Tenge Kazakstan",
          },
        },
      },
    ],
    SZL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Emalangeni Swaziland",
            tekst: "Emalangeni Swaziland",
          },
        },
      },
    ],
    LTL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Litauen",
            tekst: "Litauen",
          },
        },
      },
    ],
    SAR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Saudi Arabia",
            tekst: "Saudi Arabia",
          },
        },
      },
    ],
    TTD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Trinidadand Tobago",
            tekst: "Dollars Trinidadand Tobago",
          },
        },
      },
    ],
    YER: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rials Yemen",
            tekst: "Rials Yemen",
          },
        },
      },
    ],
    MVR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rufiyaa Maldives (Maldive Islan",
            tekst: "Rufiyaa Maldives (Maldive Islan",
          },
        },
      },
    ],
    INR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Indiske Rupi",
            tekst: "Indiske Rupi",
          },
        },
      },
    ],
    KRW: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Sør-Korea",
            tekst: "Sør-Korea",
          },
        },
      },
    ],
    AWG: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Guilders Aruba",
            tekst: "Guilders Aruba",
          },
        },
      },
    ],
    NPR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Nepal Rupees Nepal",
            tekst: "Nepal Rupees Nepal",
          },
        },
      },
    ],
    JPY: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Japanske Yen",
            tekst: "Japanske Yen",
          },
        },
      },
    ],
    MNT: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tugriks Mongolia",
            tekst: "Tugriks Mongolia",
          },
        },
      },
    ],
    PLN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Polen",
            tekst: "Polen",
          },
        },
      },
    ],
    AOA: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kwanza Angola",
            tekst: "Kwanza Angola",
          },
        },
      },
    ],
    GBP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Britiske pund",
            tekst: "Britiske pund",
          },
        },
      },
    ],
    SBD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Solomon Islands",
            tekst: "Dollars Solomon Islands",
          },
        },
      },
    ],
    CSD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dinars Serbia",
            tekst: "Dinars Serbia",
          },
        },
      },
    ],
    HUF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Ungarn",
            tekst: "Ungarn",
          },
        },
      },
    ],
    BYR: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Rubles Belarus",
            tekst: "Rubles Belarus",
          },
        },
      },
    ],
    BIF: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Francs Burundi",
            tekst: "Francs Burundi",
          },
        },
      },
    ],
    MWK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Kwachas Malawi",
            tekst: "Kwachas Malawi",
          },
        },
      },
    ],
    MGA: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Ariary Madagascar",
            tekst: "Ariary Madagascar",
          },
        },
      },
    ],
    BZD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Belize",
            tekst: "Dollars Belize",
          },
        },
      },
    ],
    BAM: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Convertible Marka Bosniaand Her",
            tekst: "Convertible Marka Bosniaand Her",
          },
        },
      },
    ],
    MOP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Patacas Macau",
            tekst: "Patacas Macau",
          },
        },
      },
    ],
    EGP: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Pounds Egypt",
            tekst: "Pounds Egypt",
          },
        },
      },
    ],
    NAD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Dollars Namibia",
            tekst: "Dollars Namibia",
          },
        },
      },
    ],
    SKK: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Slovakia",
            tekst: "Slovakia",
          },
        },
      },
    ],
    TMM: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Manats Turkmenistan",
            tekst: "Manats Turkmenistan",
          },
        },
      },
    ],
    NIO: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Gold Cordobas Nicaragua",
            tekst: "Gold Cordobas Nicaragua",
          },
        },
      },
    ],
    PEN: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Nuevos Soles Peru",
            tekst: "Nuevos Soles Peru",
          },
        },
      },
    ],
    WST: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Tala Samoa",
            tekst: "Tala Samoa",
          },
        },
      },
    ],
    NZD: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "New Zealand dollar",
            tekst: "New Zealand dollar",
          },
        },
      },
    ],
    BRL: [
      {
        gyldigFra: "1900-01-01",
        gyldigTil: "9999-12-31",
        beskrivelser: {
          nb: {
            term: "Brazil Real Brazil",
            tekst: "Brazil Real Brazil",
          },
        },
      },
    ],
  },
};

export function getCurrencies(lang = "nb") {
  const currencyObj = Object.keys(currencies.betydninger);
  const mostUsedCurr = [];
  const currencyList = [];
  const compareAscending = (a, b, locale) => a.localeCompare(b, locale);

  currencyObj.forEach((key, index) => {
    let currencyName = `${currencies.betydninger[key][0].beskrivelser.nb.tekst}`;
    let newObj = { label: `${currencyName} (${key})`, value: `${key}` };
    if (`${key}` === "NOK" || `${key}` === "EUR" || `${key}` === "SEK") {
      mostUsedCurr.push(newObj);
    }
    //Fordeler/ulemper ved å sette else rundt currencyList.push(..) under
    // Mot: hvis mest brukte valutaer ikke ligger dobbelt opp vil noen mulig ikke finne landet i a-å-rekkefølgen
    // For: hvis mest brukte valutaer ligger dobbelt dukker det opp to like resultater i autosøket
    currencyList.push(newObj);
  });

  sortAsc(currencyList);
  sortAsc(mostUsedCurr);

  function sortAsc(arr) {
    let sortedarr = arr.sort((a, b) => compareAscending(a.label.toUpperCase(), b.label.toUpperCase(), lang));
    return sortedarr;
  }

  const options = mostUsedCurr.concat(currencyList);

  return options;
}
