...
import {validateEarliestAndLatestDate, validateToAndFromDate} from "./NavDatepicker";
  ...
  describe('validateToAndFromDate', () => {
    ...
    describe("When mayBeEqual is true", () => {
      it("Fails with appropriate message when inputDate is earlier than from-date", () => {
        expect(validateToAndFromDate(fromDate, earlierThanFromDate, true))
          .toBe("Datoen kan ikke være tidligere enn fra-dato (31.12.2030)");
      });
    ...
    describe("When mayBeEqual is false", () => {
      it("fails with appropriate message when input is earlier than from-date", () => {
        expect(validateToAndFromDate(fromDate, earlierThanFromDate, false))
          .toBe("Datoen må være senere enn fra-dato (31.12.2030)")
      });
  ...
  describe("validateEarliestAndLatestDate", () => {
    describe("When earliestFromToday and latestFromToday are set", () => {
      ...
      it("fails with appropriate message if input is before earliest date", () => {
        expect(validateEarliestAndLatestDate(1, 2, moment()))
          .toBe("Datoen kan ikke være tidligere enn 16.05.2030 eller senere enn 17.05.2030");
      });

      it("fails with appropriate message if input is after latest date", () => {
        expect(validateEarliestAndLatestDate(-2, -1, moment()))
          .toBe("Datoen kan ikke være tidligere enn 13.05.2030 eller senere enn 14.05.2030");
      });
    ...
    describe("When earliestFromToday is set", () => {
      it("fails with appropriate message if input is before earliest date", () => {
        expect(validateEarliestAndLatestDate(1, undefined, moment()))
          .toBe("Datoen kan ikke være tidligere enn 16.05.2030");
      });
    ...
    describe("When latestFromToday is set", () => {
      it("fails with appropriate message if input is after latest date", () => {
        expect(validateEarliestAndLatestDate(undefined, -1, moment()))
          .toBe("Datoen kan ikke være senere enn 14.05.2030");
      });
