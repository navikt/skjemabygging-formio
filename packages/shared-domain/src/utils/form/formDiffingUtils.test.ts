import { Component, Form, FormPropertiesType, NavFormType } from '../../models';
import { DiffStatus, formDiffingUtils } from './formDiffingUtils';
import { navFormUtils } from './navFormUtils';
import testdataPublishedForm from './testdata/diff/published-form';
import form from './testdata/nav-form/nav100750';

describe('formDiffingUtils', () => {
  it('Diff same form', () => {
    const changes = formDiffingUtils.generateNavFormDiff(form, form);
  ...
  describe('generateNavFormSettingsDiff', () => {
    ...
    it('reports diff on form title', () => {
      ...
      const diff = formDiffingUtils.generateNavFormSettingsDiff(publishedForm, testform);
      expect(diff).toEqual({
        title: {
    ...
    it('ignores signatures on old format', () => {
      ...
      const diff = formDiffingUtils.generateNavFormSettingsDiff(publishedFormWithOldSignatures, testform);
      expect(diff).toEqual({
  ...
  describe('Diff against changes', () => {
    ...
    it('Diff modified date', () => {
      ...
      const changes: any = formDiffingUtils.generateNavFormDiff(form, newForm);
  ...
  describe('checkComponentDiff', () => {
    ...
    it('has no diff when form is not published', () => {
      ...
      const changes = formDiffingUtils.checkComponentDiff(comp, undefined);
      expect(changes).toBeNull();
    });

    it('has no diff when component is not changed', () => {
      const comp = getComp('fornavn');
      const changes = formDiffingUtils.checkComponentDiff(comp, publishedForm);
