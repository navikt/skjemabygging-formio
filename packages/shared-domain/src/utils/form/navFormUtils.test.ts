...
import formWithSkjemagruppe from './testdata/nav-form/conditional-skjemagruppe';
import nav100750 from './testdata/nav-form/nav100750';
...
describe('navFormUtils', () => {
  describe('toFormPath', () => {
    it('should create path from skjemanummer', () => {
      expect(navFormUtils.toFormPath('NAV 10-13.76')).toBe('nav101376');
    });

    it('should handle øæå', () => {
      expect(navFormUtils.toFormPath('ØÆÅ 10-13.76')).toBe('oaea101376');
    });

    it('should create path from title (legacy)', () => {
      expect(
        navFormUtils.toFormPath(
      ...
      expect(navFormUtils.toFormPath('Søknad om forlenget barnepensjon etter fylte 18 år')).toBe(
        'soknadomforlengetbarnepensjonetterfylte18ar',
      );
      expect(navFormUtils.toFormPath('Søknad om utstedelse av attest PD U2')).toBe('soknadomutstedelseavattestpdu2');
      expect(navFormUtils.toFormPath('Underveis- og sluttevaluering av AMO-KURS')).toBe(
        'underveisogsluttevalueringavamokurs',
      );
    });
  });
  ...
  describe('formMatcherPredicate', () => {
    ...
    describe('A form where path is derived from title (legacy)', () => {
      it('should match the path', () => {
        ...
        expect(navFormUtils.formMatcherPredicate('firsttestform')(form)).toBe(true);
      });

      it('should match the skjemanummer', () => {
        const form = createForm('First test form', 'firsttestform', 'NAV 12-34.56');
        expect(navFormUtils.formMatcherPredicate('nav123456')(form)).toBe(true);
      });

      it('should not match other skjemanummer', () => {
        const form = createForm('First test form', 'firsttestform', 'NAV 12-34.56');
        expect(navFormUtils.formMatcherPredicate('nav654321')(form)).toBe(false);
      });

      it('should not match other title', () => {
        const form = createForm('First test form', 'firsttestform', 'NAV 12-34.56');
        expect(navFormUtils.formMatcherPredicate('secondtestform')(form)).toBe(false);
      });
    });
    ...
    describe('A form where the path is derived from skjemanummer', () => {
      ...
      it('should match the title (legacy)', () => {
        const form = createForm('Second test form', 'nav123456', 'NAV 12-34.56');
        expect(navFormUtils.formMatcherPredicate('secondtestform')(form)).toBe(true);
      });
  ...
  describe('flattenComponents', () => {
    it('returns a flat array of all nested components', () => {
      const actual = navFormUtils.flattenComponents([
        {
  ...
  describe('findDependentComponents', () => {
    describe('A form where one component has a simple conditional on another component', () => {
      it('Returns empty array when component has no conditional', () => {
        const actual = navFormUtils.findDependentComponents('ec464ka', formWithSimpleConditional);
        expect(actual).toHaveLength(0);
      });

      it('Returns an array with the key of the component it has a conditional on', () => {
        const actual = navFormUtils.findDependentComponents('ekoo75nf', formWithSimpleConditional);
    ...
    describe('A form where one component has a custom conditional', () => {
      it('Returns empty array when component has no conditional', () => {
        const actual = navFormUtils.findDependentComponents('e794rrs', formWithCustomConditional);
        expect(actual).toHaveLength(0);
      });

      it('Returns an array with the key of the component it has a conditional on', () => {
        const actual = navFormUtils.findDependentComponents('ekoo75nf', formWithCustomConditional);
        const expected = [expect.objectContaining({ key: 'oppgiYndlingsfarge' })];
        expect(actual).toEqual(expect.arrayContaining(expected));
        expect(actual).toHaveLength(expected.length);
      });
    ...
    describe('A form where one component has a custom conditional with dependency to several components', () => {
      it('when given the id of the first component in the conditional it returns the key of the component owning the conditional', () => {
        const actual = navFormUtils.findDependentComponents('em5trf1', formWithCompositeCustomConditional);
      ...
      it('when given the id of the second component in the conditional it returns the key of the component owning the conditional', () => {
        const actual = navFormUtils.findDependentComponents('eoql8pp', formWithCompositeCustomConditional);
        const expected = [expect.objectContaining({ key: 'hvisSolbrillerOgSolkremEllerSolseng' })];
        expect(actual).toEqual(expect.arrayContaining(expected));
        expect(actual).toHaveLength(expected.length);
      });
      ...
      it('when given the id of the last component in the conditional it returns the key of the component owning the conditional', () => {
        const actual = navFormUtils.findDependentComponents('eo9rcfe', formWithCompositeCustomConditional);
        const expected = [expect.objectContaining({ key: 'hvisSolbrillerOgSolkremEllerSolseng' })];
        expect(actual).toEqual(expect.arrayContaining(expected));
        expect(actual).toHaveLength(expected.length);
      });
    ...
    describe('component=a inside datagrid with custom conditional referring to component=b inside the same datagrid using row variable', () => {
      it('returns component=a when checking component=b inside the datagrid', () => {
        const actual = navFormUtils.findDependentComponents('b', formWithDatagridConditional);
    ...
    describe('A form with a container containing a component with same key as component outside container', () => {
      ...
      it('Returns dependent component for textfield inside container', () => {
        ...
        const actual = navFormUtils.findDependentComponents(MYTEXTFIELD_INSIDE_CONTAINER.id, form);
      ...
      it('Returns dependent component for containter containing textfield', () => {
        const form = testform(conditional({ show: 'true', when: 'mycontainer.mytextfield', eq: 'yeah' }));
        const actual = navFormUtils.findDependentComponents(CONTAINER.id, form);
        const expected = [expect.objectContaining({ key: COMPONENT_WITH_CONDITIONAL.key })];
        expect(actual).toHaveLength(expected.length);
        expect(actual).toEqual(expect.arrayContaining(expected));
      });
      ...
      it('Returns no dependent component for textfield outside container', () => {
        const form = testform(conditional({ show: 'true', when: 'mycontainer.mytextfield', eq: 'yeah' }));
        const actual = navFormUtils.findDependentComponents(MYTEXTFIELD_OUTSIDE_CONTAINER.id, form);
        expect(actual).toHaveLength(0);
      });
    });
    ...
    describe('A form with a datagrid containing textfield with same key as textfield outside datagrid', () => {
      ...
      it('Returns dependent component for textfield inside datagrid for simple conditional', () => {
        ...
        const actual = navFormUtils.findDependentComponents(MYTEXTFIELD_INSIDE_DATAGRID.id, form);
        const expected = [expect.objectContaining({ key: COMPONENT_WITH_CONDITIONAL.key })];
        expect(actual).toHaveLength(expected.length);
        expect(actual).toEqual(expect.arrayContaining(expected));
      });
      ...
      it('Returns dependent component for datagrid when referenced in custom conditional', () => {
        ...
        const actual = navFormUtils.findDependentComponents(DATAGRID.id, form);
        const expected = [expect.objectContaining({ key: COMPONENT_WITH_CONDITIONAL.key })];
        expect(actual).toHaveLength(expected.length);
        expect(actual).toEqual(expect.arrayContaining(expected));
      });
      ...
      it('Returns no dependent component for textfield outside datagrid', () => {
        const form = testform(conditional({ show: 'true', when: 'myDataGrid.mytextfield', eq: 'yeah' }));
        const actual = navFormUtils.findDependentComponents(MYTEXTFIELD_OUTSIDE_DATAGRID.id, form);
        expect(actual).toHaveLength(0);
      });
    });
    ...
    describe('A form with multiple conditional dependencies', () => {
      ...
      it("returns three dependent component keys for 'frukt'", () => {
        const actual = navFormUtils.findDependentComponents('e5wjypl', testForm);
        const expected = [
      ...
      it("returns two dependent component keys for 'likerDuFrukt'", () => {
        const actual = navFormUtils.findDependentComponents('efc5w41', testForm);
      ...
      it('returns no dependent keys for components with no conditional', () => {
        expect(navFormUtils.findDependentComponents('e0p4b08', testForm)).toHaveLength(0);
        expect(navFormUtils.findDependentComponents('enfbr1xh', testForm)).toHaveLength(0);
        expect(navFormUtils.findDependentComponents('epr77lc', testForm)).toHaveLength(0);
      });

      it('does not return components downstream to given key', () => {
        const actual = navFormUtils.findDependentComponents('e1xrfj', testForm);
        expect(actual).toHaveLength(0);
      });

      it('Returns empty array for unknown key', () => {
        const actual = navFormUtils.findDependentComponents('ukjentid', testForm);
        expect(actual).toHaveLength(0);
      });
    });
      ...
      testForms.forEach((testForm) => {
        describe(`Grouped with ${testForm.title}`, () => {
          it("returns two dependenct component keys for 'oppgiYndlingsfarge'", () => {
            const dependentKeys = navFormUtils.findDependentComponents('e83xe9j', testForm);
            const actual = [
          ...
          it("returns one dependenct component keys for 'hvilkenGronnFruktLikerDuBest'", () => {
            const actual = navFormUtils.findDependentComponents('eq4be9', testForm);
          ...
          it("returns one dependenct component keys for 'minGruppering'", () => {
            const actual = navFormUtils.findDependentComponents('e34565l', testForm);
            const expected = [expect.objectContaining({ key: 'alertstripe' })];
            expect(actual).toEqual(expect.arrayContaining(expected));
            expect(actual).toHaveLength(expected.length);
          });
  ...
  describe('removeComponent', () => {
    ...
    it('removes vedleggspanel', () => {
      const regex = /.*Vedlegg.*/;
      const actualForm = navFormUtils.removeComponents(
        form,
    ...
    it('removes both html components', () => {
      const actualForm = navFormUtils.removeComponents(form, (component) => component.type === 'html');
  ...
  describe('removeVedleggspanel', () => {
    it('removes vedleggspanel with param isAttachmentPanel', () => {
      const actualForm = navFormUtils.removeVedleggspanel({
        components: [
          {
            type: 'panel',
            key: 'vedlegg',
    ...
    it('A form with a component that calculates value based on another', () => {
      ...
      const actual = navFormUtils.findDependentComponents(
        'navCheckbox1',
        formWithComponentsThatCalulateValueBasedOnAnother,
      );
    ...
    it('A form with a component that validates based on another', () => {
      ...
      const actual = navFormUtils.findDependentComponents(
        'navCheckbox1',
        formWithComponentsThatValidatesBasedOnAnother,
      );
      expect(actual).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
  ...
  describe('isSubmissionMethodAllowed', () => {
    ...
    describe('submissionTypes=["PAPER"]', () => {
      it('paper is allowed', () => {
        ...
        const allowed = navFormUtils.isSubmissionMethodAllowed('paper', testform);
        expect(allowed).toBe(true);
      });

      it('digital is not allowed', () => {
        const testform = createTestForm(['PAPER']);
        const allowed = navFormUtils.isSubmissionMethodAllowed('digital', testform);
  ...
  describe('isEqual', () => {
    it('compares two forms and skips certain properties', () => {
      expect(navFormUtils.isEqual(nav100750, withDifferentModified)).toBe(false);
      expect(navFormUtils.isEqual(nav100750, withDifferentModified, ['modified'])).toBe(true);
    });

    it('ignores ordering of properties', () => {
      expect(navFormUtils.isEqual(nav100750, withReOrderedProperties)).toBe(true);
    });
  });
  ...
  describe('enrichComponentsWithNavIds', () => {
    ...
    it('ignores undefined input', () => {
      const components = navFormUtils.enrichComponentsWithNavIds(undefined, navIdGenerator);
    ...
    it('does not update existing navId', () => {
      ...
      const components = navFormUtils.enrichComponentsWithNavIds(input, navIdGenerator);
