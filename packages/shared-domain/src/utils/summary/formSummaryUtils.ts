  ...
  switch (component.type) {
    ...
    case 'day': {
      if (value.match('00/00/')) {
        ...
        return `${translate(stringUtils.toPascalCase(month))}, ${validValue.format('YYYY')}`;
      }
    }
    case 'currency':
    ...
    case 'iban': {
      return formatUtils.formatIBAN(value);
    }
    case 'orgNr': {
    ...
    case 'fnrfield': {
      return opts.skipPdfFormatting ? value : formatUtils.formatNationalIdentityNumber(value);
    }
    case 'number': {
    ...
    case 'monthPicker':
      return stringUtils.toPascalCase(dateUtils.toLongMonthFormat(value, language));
    default:
      return value;
  }
}
...
function handleField(
  ...
  const submissionValue = navFormioUtils.getValue(submission, componentKey);
  if (
...
function handleDataGridRows(component, submission, translate, form, language, opts) {
  ...
  const dataGridSubmission = navFormioUtils.getValue(submission, key) || [];
...
const handleDataFetcher = (component, submission, formSummaryObject, parentContainerKey, translate) => {
  const { key, label, type } = component;
  const componentKey = createComponentKey(parentContainerKey, key);
  const fetcher = dataFetcherUtils.dataFetcher(componentKey, submission);
  if (!fetcher.success) {
    return formSummaryObject;
  }
