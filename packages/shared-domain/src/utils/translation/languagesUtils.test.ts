import { languageUtils } from './languagesUtils';
...
describe('languagesUtil', () => {
  describe('globalEntitiesToI18nGroupedByTag', () => {
    it('converts global entities and groups by tag', () => {
      const i18n = languageUtils.globalEntitiesToI18nGroupedByTag([
        globalGrensesnittEn,
        globalValideringEn,
  ...
  describe('flattenGlobalI18nGroupedByTag', () => {
    it('flattens', () => {
      ...
      const i18n = languageUtils.flattenGlobalI18nGroupedByTag(i18nGroupedByTag);
      expect(Object.keys(i18n)).toEqual(['en', 'nn-NO']);
