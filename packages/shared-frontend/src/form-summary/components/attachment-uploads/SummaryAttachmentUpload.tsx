...
const SummaryAttachmentUpload = (props: FormComponentProps) => {
  const { component, submission, translate, formProperties, rendererConfig, handleDownloadFile } = props;
  const { label } = component;
  const attachmentDownloadEnabled = enableAttachmentDownload(rendererConfig.submissionMethod);
        ...
        {submissionAttachments.map((submissionAttachment) =>
          hasUploadedFiles(submissionAttachment) ? (
            ...
            <FormSummary.Value key={submissionAttachment.attachmentId}>
              <VStack gap="space-8">
                {translate(
                  attachmentUtils.getAttachmentLabel(submissionAttachment.value!, rendererConfig.submissionMethod),
                )}
