...
import NavFormBuilder from "../src/components/NavFormBuilder";
...
function onNext(cmp, event, cb) {
  if (typeof cmp.events !== "object") {
    throw Error("events skal være object");
  }
  if (typeof cmp.events.once !== "function") {
    throw Error("events.once skal være function");
  }
...
const Harness = {
  builderBefore(done, options = {}) {
    ...
    formBuilder = new NavFormBuilder(formBuilderElement, { display: "form", components: [] }, options);
  ...
  testComponent(component, test, done) {
    ...
    component.on("componentError", (error) => {
      console.log("testComponent componentError error", error);
      if (!testBad) {
